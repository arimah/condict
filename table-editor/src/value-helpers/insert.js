...
export const INSERT_START = -2;
export const INSERT_BEFORE = -1;
export const INSERT_AFTER = 1;
export const INSERT_END = 2;

const getInsertIndex = (location, selMin, selMax, tableSize) => {
  switch (location) {
    case INSERT_START:
      return [0, 0];
    case INSERT_BEFORE:
    ...
    default:
      throw new Error(`Invalid insert location: ${location}`);
  }
};

export const insertRow = (value, location) => {
  ...
  for (let c = 0; c < layout.colCount; ) {
    ...
    if (needsExtension) {
      newRows = newRows.updateIn(pathToCell(layoutCell), cell =>
...
export const insertColumn = (value, location) => {
  const {selection, layout} = value;
  ...
  for (let r = 0; r < layout.rowCount; ) {
    ...
    if (needsExtension) {
      ...
      newRows = newRows.updateIn([r, 'cells'], cells =>
